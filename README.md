...
- **Current Implementation**: Google Gemma-3-4B-IT (configurable to Gemma-2-2B for testing or Gemma-2-9B for larger runs)
...
- **Current Setting**: `dropconnect_p: 0.0` (disabled for stability, can be enabled for experimentation)
...
Training CI-LLM presents significant computational challenges that necessitate high-performance computing infrastructure:

#### 1. **Model Scale Complexity**
- **Base Model**: Gemma-3-4B has 4 billion parameters (currently deployed)
...
#### 2. **Why Scalable Computing is Essential**
...
#### 1. **Model Architecture** (`src/models/gemma_backbone.py`)
...
#### 2. **Diversity Regularization** (`src/losses/sliced_w2.py`)
...
#### 3. **Bayesian Aggregation** (`src/aggregator/dirichlet_bayesian.py`)
...
#### 4. **Training Pipeline** (`src/training/train.py`)
...
│   ├── default.yaml              # Main configuration (Gemma-3-4B)
...
accelerate launch --config_file configs/accelerate_deepspeed_config.yaml \
    src/training/train.py --config configs/default.yaml
```
...
- `configs/default.yaml`: Main training configuration (Gemma-3-4B, K=4)
- `configs/full_train.yaml`: Alternative configuration (Gemma-2-2B, K=4)
...
#### 2. **Multi-GPU Training with DeepSpeed**
```bash
...
- **Model**: Gemma-3-4B with K=4 agents
